...
public class InputHandlerTablet extends InputHandlerGeneric {
	...
	public InputHandlerTablet(RemoteCanvasActivity activity, RemoteCanvas canvas,
	...
	public String getDescription() {
		return canvas.getResources().getString(R.string.input_method_direct_tablet_description);
	}
...
//			Log.d("tablet", "scroll! distanceX: " + distanceX + " distanceY: " + distanceY);
		if (scrollMode){
			...
			scrollRight = false;
			scrollLeft  = false;
			if (abs(distanceY) > abs(distanceX)){ // vertical movement
				if (distanceY < -5) scrollUp = true;
				else if (distanceY > 5) scrollDown = true;
			}else if(abs(distanceY) < abs(distanceX)){
				if (distanceX < -5) scrollLeft = true;
				else if (distanceX > 5) scrollRight = true;
			}
		}
...
//				Log.d("tablet", "UP!" + "Duration: " + touchDuration + " longDuration: " + longPressTimeout);
				p.moveMouseButtonUp(getX(e), getY(e), e.getMetaState()); // to end drag effect
				resetAllModes();
				return true;
