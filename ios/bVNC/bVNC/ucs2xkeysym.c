...
int ucs2keysym (const unsigned char *chr) {
    unsigned long int ucs = chr[0] | (chr[1] << 8) | (chr[2] << 16) | (chr[3] << 24);
    ...
    if ((ucs >= 0x0020 && ucs <= 0x007e) ||
        ...
        printf("Direct conversion to: ", ucs);
        return ucs;
    }
    while (max >= min) {
        mid = (min + max) / 2;
        if (keysymtabbyucs[mid].ucs < ucs)
            ...
            printf("Found %d", keysymtabbyucs[mid].ucs);
            return keysymtabbyucs[mid].ucs;
        }
    }
