...
#else
    sprintf(file_str, "/data/data/com.keqisoft.android.spice/ahoo-%u.jpg", id);
#endif

    FILE *f = fopen(file_str, "wb");
    if (!f) {
    SPICE_DEBUG("DUMP_JPEG failed");
        return;
    }
...
#ifdef DUMP_JPEG
    dump_jpeg(image->u.jpeg.data->chunk[0].data, image->u.jpeg.data->chunk[0].len);
#endif
    return surface;
}
