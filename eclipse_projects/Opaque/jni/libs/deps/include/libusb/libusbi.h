...
struct usbi_os_backend {
	...
	void (*close_fd)(struct libusb_device_handle *handle,int fd);
	...
	int (*get_device_descriptor_fd)(struct libusb_device *device,
		unsigned char *buffer, int *host_endian, int fd);
	...
	int (*get_config_descriptor_fd)(struct libusb_device *device,
		uint8_t config_index, unsigned char *buffer, size_t len,
		int *host_endian, int fd);
